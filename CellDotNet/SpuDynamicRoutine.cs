...
//						}
						...
						// The second last basicblock contains the inner epilog(restor callee saves register), 
						// which is generated by the register allocator.
						int epilogBlockIndex = bblist.Count-2;
						if (bbindex != epilogBlockIndex - 1)
						{
							inst.JumpTarget = bblist[epilogBlockIndex];
							branchlist.Add(new KeyValuePair<int, SpuInstruction>(curroffset, inst));
						}
						else
						{
							// Might as well avoid the branch to the next instruction..
							inst.OpCode = SpuOpCode.nop;
						}
					}
